...
--*/
{
	...
	Rxdif = (end.QuadPart - start.QuadPart);
	RxTotal += Rxdif;
  ...
  //  DbgPrint("Receive\n");
	...
	//	DbgPrint("While\n");
		...
		//	DbgPrint("2\n");
            ...
            else if(pEthHeader->DstAddr[5] != 1)
			{

				break;
			}

            if(pEthHeader->DstAddr[5] == 1)
            {
            	 ndisprotAllocateAndSendNetBufferList(pOpenContext);
            	 //bAcceptedReceive = TRUE;
            	// DbgPrint("Ndisprot: RECIEVE\n");
            	 break;
            }
            bAcceptedReceive = TRUE;
...
--*/
{
 ...
 //   DbgPrint("==>Send\n");
    do
    {
        ...
        if (pDataBuffer == NULL)
        {
            DEBUGP(DL_FATAL, ("AllocSendNbl: open %p, failed to alloc"
        ...
        if (pNetBufList == NULL)
        {
            DEBUGP(DL_FATAL, ("AllocateSendNbl: open %p, failed to alloc"
                " Net Buffer List, %d bytes\n", pOpenContext, DataLength));
            break;
        }

        NBL_SET_PROT_RSVD_FLAG(pNetBufList, NPROT_ALLOCATED_NBL);
        NPROT_SEND_NBL_RSVD(pNetBufList)->RefCount = 1;


        pNetBufList->SourceHandle = pOpenContext->BindingHandle;
