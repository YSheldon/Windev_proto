...
--*/
{
    ...
    PADAPT Adapter = (PADAPT)FunctionContext;
    ...
    if (fSend == TRUE)
    {
        ...
        NdisSetTimerObject(Adapter->Timer, liDelay, 0, NULL);
...
pAdapt - pointer to adapter on which the packets has to be sent
...
--*/
{
    ...
    do
    {
        OPLK_ALLOC_MEM(pDataBuffer, DataLength);

        //DataLength = DATA_LENGTH;
        if (pDataBuffer == NULL)
        {
            DbgPrint("AllocSendNbl: open %p, failed to alloc data buffer %d bytes\n", pAdapt, DataLength);
            break;
        }
        RtlCopyMemory(pDataBuffer, aframe, DataLength);
        //
        //  Make this an NDIS buffer.
        //
#pragma prefast(suppress:6014, "The instance is freed by calling NdisFreeMdl(pMdl)")
        pMdl = NdisAllocateMdl(pAdapt->BindingHandle, pDataBuffer, DataLength);

        if (pMdl == NULL)
        {
            DbgPrint("AllocateRcvNbl: open %p, failed to alloc MDL, %d bytes\n", pAdapt, DataLength);
            break;
        }
        ASSERT(pAdapt->SendNetBufferListPool != NULL);
        pNetBufList = NdisAllocateNetBufferAndNetBufferList(
            pAdapt->SendNetBufferListPool,  // NetBufferPool Handle
            0,                              // ContextSize
            0,                              // ContextBackfill
            pMdl,                           // MdlChain
            0,                              // DataOffset
        ...
        if (pNetBufList == NULL)
        {
            DbgPrint("AllocateSendNbl: open %p, failed to alloc Net Buffer List, %d bytes\n", pAdapt, DataLength);
            break;
        }

        NBL_SET_PROT_RSVD_FLAG(pNetBufList, OPLK_ALLOCATED_NBL);


        pNetBufList->SourceHandle = pAdapt->BindingHandle;
...
//    POBJECT_NAME_INFORMATION nameInfo = NULL;
 ...
 //   LARGE_INTEGER                   liDelay;
    ...
    do
    {
        ...
        if (STATUS_SUCCESS == RtlUnicodeStringToAnsiString(&strVname, BindParameters->AdapterName, TRUE))
        {
            strcpy(PtrFullPathFname, strVname.Buffer);
            RtlFreeAnsiString(&strVname);
        }
        DbgPrint("Adapter Name %s\n", PtrFullPathFname);
        ...
        PoolParameters.PoolTag = MUX_TAG;
       
        if (ProtHandle != NULL)
        {
            //DbgPrint("Using Protocol Handle\n");
        }
        else
        {
            //DbgPrint("Handle NUll\n");
        }

        pAdapt->SendNetBufferListPool = NdisAllocateNetBufferListPool(
            ProtHandle,
            &PoolParameters);
        if (pAdapt->SendNetBufferListPool == NULL)
        {
        ...
        Timer.TimerFunction = TimerDpc;
        Timer.FunctionContext = pAdapt;
        Timer.AllocationTag = MUX_TIMER_TAG;
...

{
    ...
    NdisFreeNetBufferListPool(pAdapt->SendNetBufferListPool);
   ...
   // DbgPrint("Receive\n");
    ...
    while (NetBufferLists != NULL)
    {
        ...
        do
        {
            ...
            // TODO: GKS: Check for the SoC here and call the send routine
...
//    UNREFERENCED_PARAMETER(NetBufferLists);
    ...
    */
    ...
    while(NetBufferLists)
    {
        ...
        if (NBL_TEST_PROT_RSVD_FLAG(CurrentNetBufferList, OPLK_ALLOCATED_NBL))
        {
            //DbgPrint("Free\n");
            pMdl = NET_BUFFER_FIRST_MDL(NET_BUFFER_LIST_FIRST_NB(CurrentNetBufferList));
            TotalLength = NET_BUFFER_DATA_LENGTH(NET_BUFFER_LIST_FIRST_NB(CurrentNetBufferList));
            ...
            ASSERT(BufferLength == TotalLength);
            ASSERT(pCopyData != NULL); // we would have allocated non-paged pool
